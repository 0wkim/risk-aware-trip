          ...
          {timeSeriesData.map((data, idx) => (
            <div key={idx} className="flex flex-col items-center flex-1 h-full justify-end group relative">
              ...
              <div 
                ...
                className={`w-4 md:w-5 rounded-t-sm transition-all duration-500 shrink-0 ${
                  idx === 1 
                    ? 'bg-gradient-to-t from-rose-500 to-rose-400 shadow-md shadow-rose-500/20' 
      ...
      <div className={`p-4 rounded-2xl shadow-lg md:shadow-2xl border backdrop-blur-md flex flex-col gap-3 text-left ${isDarkMode ? 'bg-slate-900/90 border-slate-800 text-white' : 'bg-white/95 border-slate-100 text-slate-800'}`}>
        ...
        <div className="flex flex-col gap-2.5 relative pl-0.5">
          ...
          <div className="flex items-center gap-2 md:gap-2.5 relative z-10 text-[9px] md:text-[10px]">
            ...
            <div className="truncate"><span className="text-slate-400 text-[7px] md:text-[8px] block font-medium">TRANSIT LINK (예측 가혹도 링크)</span><span className="text-slate-400 font-bold">확률 밀도 함수 결합 추정 중</span></div>
          </div>
    ...
    <div className={`flex flex-col md:flex-row h-[100dvh] md:h-screen animate-in fade-in slide-in-from-right-10 duration-700 ${isDarkMode ? 'bg-[#0F172A]' : 'bg-white'}`}>
      ...
      <div className={`order-2 md:order-1 w-full md:w-[400px] h-[55vh] md:h-full flex flex-col border-t md:border-t-0 md:border-r z-30 md:z-10 shadow-[0_-10px_40px_rgba(0,0,0,0.08)] md:shadow-2xl transition-all duration-500 rounded-t-[2rem] md:rounded-none relative -mt-6 md:mt-0 ${isDarkMode ? 'bg-[#1E293B] border-slate-700' : 'bg-white border-slate-100'}`}>
        ...
        <div className="flex-1 p-5 md:p-6 space-y-5 md:space-y-6 overflow-y-auto custom-scrollbar text-left">
            ...
            <div className="flex gap-2.5 md:gap-3">
              ...
              <div className="space-y-3.5 md:space-y-4 w-full overflow-hidden">
                ...
                <div className="flex flex-col gap-1.5 md:gap-2 pb-2.5 md:pb-3 border-b border-slate-500/10 w-full">
                  <div className={`flex items-center gap-2 px-2.5 md:px-3 py-1.5 rounded-lg md:rounded-xl font-black shadow-sm text-[11px] md:text-xs w-full ${isDarkMode ? 'bg-blue-950/60 text-blue-400 border border-blue-800/50' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse shrink-0" />
                    <span className="opacity-60 text-[8px] md:text-[9px] font-medium uppercase shrink-0">출발</span>
                    <span className="tracking-tight truncate block flex-1 text-left">{startPoint}</span>
                  </div>
                  
                  <div className="flex items-center pl-3 md:pl-4 text-slate-400/80 dark:text-slate-500/80 my-0">
                    <ArrowRight size={12} className="transform rotate-90 md:w-3 md:h-3 w-2.5 h-2.5" />
                  </div>
                  
                  <div className={`flex items-center gap-2 px-2.5 md:px-3 py-1.5 rounded-lg md:rounded-xl font-black shadow-sm text-[11px] md:text-xs w-full ${isDarkMode ? 'bg-rose-950/60 text-rose-400 border border-rose-800/50' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>
                    <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse shrink-0" />
                    <span className="opacity-60 text-[8px] md:text-[9px] font-medium uppercase shrink-0">도착</span>
                    <span className="tracking-tight truncate block flex-1 text-left">{destination}</span>
                  </div>
                </div>

                {/* AI 분석 코멘트 */}
                <div className="font-bold text-xs md:text-sm leading-relaxed text-left">
                  <span className={`block text-[9px] md:text-[10px] font-black uppercase tracking-wildest mb-0.5 ${themeStyles.text}`}>AI 분석 코멘트</span>
                  <p className={`text-[11px] md:text-xs ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>{themeStyles.message}</p>
                </div>
                ...
                <div className={`p-3 md:p-3.5 rounded-xl md:rounded-2xl border text-[11px] md:text-xs font-semibold flex flex-col gap-1.5 md:gap-2 ${isDarkMode ? 'bg-slate-900/60 border-slate-800 text-slate-300' : 'bg-white border-slate-200/70 text-slate-700'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-1.5 md:gap-2 text-[10px] md:text-[11px]">
                    ...
                    <div className="flex items-center gap-1 md:gap-1.5">
                      <Clock size={10} className="text-amber-500 md:w-3 md:h-3" />
                      <span>예상 대기: <b className="font-black text-inherit">{originalMetrics.t_wait}분</b></span>
                    </div>
                    <div className="flex items-center gap-1 md:gap-1.5 col-span-2 pt-1 border-t border-dashed border-slate-500/10">
                      <Navigation size={10} className="text-rose-500 md:w-3 md:h-3" />
                      <span>도착 시 예측 혼잡도: <b className="font-black text-rose-500">{originalMetrics.congestion}%</b></span>
                    </div>
                  </div>
                </div>
                
                {/* AI 권장 이동수단 뱃지 */}
                <div className={`flex items-center gap-2 p-2 md:p-3 rounded-xl md:rounded-2xl border ${isDarkMode ? 'bg-slate-900/60 border-slate-800' : 'bg-white border-slate-200/60'}`}>
                  <span className={`p-1.5 md:p-2 rounded-lg md:rounded-xl border flex items-center justify-center shrink-0 ${detectedVehicle.themeClass}`}>
                    {detectedVehicle.icon}
                  </span>
                  <div className="text-left">
                    <span className="block text-[7px] md:text-[8px] font-black uppercase tracking-wider text-slate-400">AI 권장 수단</span>
                    <span className={`text-[11px] md:text-xs font-black ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{detectedVehicle.label}</span>
                  </div>
                </div>

                {/* 가용 시간 예산 정보 */}
                <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-slate-500/10 text-[10px] md:text-[11px]">
                   <span className="px-1.5 py-0.5 rounded bg-slate-500/10 border border-slate-500/20 text-slate-400 text-[8px] md:text-[9px] font-black uppercase">가용 예산</span>
                   <span className="font-black text-slate-500 mr-0.5 md:mr-1">{maxHours}시간 {maxMinutes}분</span>
                   <span className="px-1.5 py-0.5 rounded bg-emerald-500 text-white text-[8px] md:text-[9px] font-black uppercase">성공률</span>
          ...
          {verdict === 'PASS' && (
            <div className={`p-4 md:p-5 rounded-xl md:rounded-2xl border flex flex-col gap-1.5 text-[11px] md:text-xs font-bold leading-relaxed transition-all ${isDarkMode ? 'bg-slate-900/40 border-slate-800 text-slate-400' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
              <div className="flex items-center gap-1.5 text-emerald-500 font-black text-[11px] md:text-xs"><Sparkles className="md:w-3.5 md:h-3.5 w-3 h-3" /> 기존 경로 유지 권장</div>
          ...
          {verdict === 'FAIL' && recommendations.length > 0 && (
            <div className="space-y-3 md:space-y-4 pb-4 animate-in fade-in duration-300">
              <h3 className="text-[10px] md:text-[11px] font-black text-slate-400 uppercase tracking-wildest ml-1">
              ...
              <div className="space-y-2.5 md:space-y-3">
                {recommendations.map((item: Recommendation) => (
                  <div 
                    ...
                    className={`group p-3.5 md:p-5 rounded-xl md:rounded-2xl border transition-all flex items-center justify-between cursor-pointer ${isDarkMode ? 'bg-slate-800/40 border-slate-700 hover:border-emerald-500/50' : 'bg-white border-slate-100 hover:border-emerald-200 hover:shadow-md'}`}
                  >
                    <div className="flex items-center gap-3 md:gap-4 text-left min-w-0 flex-1">
                      <div className={`w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl flex items-center justify-center shrink-0 transition-all duration-300 ${isDarkMode ? 'bg-slate-700 text-slate-300 group-hover:bg-emerald-500 group-hover:text-white' : 'bg-slate-50 text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600'}`}>
                        {detectedVehicle.mode === 'car' ? <Car size={16} className="md:w-[18px] md:h-[18px]" /> : 
                         detectedVehicle.mode === 'walking' ? <Footprints size={16} className="md:w-[18px] md:h-[18px]" /> : <Train size={16} className="md:w-[18px] md:h-[18px]" />}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 md:gap-2 mb-1">
                          <span className={`text-[8px] md:text-[9px] font-black px-1.5 py-0.5 rounded border border-current ${isDarkMode ? 'text-emerald-400/70' : 'text-emerald-600/70'}`}>{item.type}</span>
                          <h4 className={`font-bold text-xs md:text-sm tracking-tight truncate max-w-[120px] md:max-w-[150px] ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>{item.name}</h4>
                        </div>
                        
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 md:gap-3 text-[10px] md:text-[11px] font-bold text-slate-500">
                          <span className="flex items-center gap-1">
                            <Clock size={10} className="text-slate-400 shrink-0 md:w-[13px] md:h-[13px]" /> 
                            이동 {Math.round(item.t_travel)}분 / 대기 {Math.round(item.t_wait)}분
                          </span>
                          <span className={`flex items-center gap-1 font-black ${themeStyles.text}`}>
                            <Navigation size={10} className="shrink-0 md:w-[13px] md:h-[13px]" /> 
                            {item.probability}
                          </span>
                        </div>
                      </div>
                    ...
                    <div 
                      ...
                      className={`px-2 py-1.5 md:px-2.5 md:py-1.5 rounded-md md:rounded-lg border text-[8px] md:text-[9px] font-black uppercase tracking-tight transition-all shrink-0 hover:scale-105 active:scale-95 ${detectedVehicle.themeClass}`}
                    >
                       선택
                    </div>
                  </div>
          ...
          {/* 모바일 화면에서만 결과 패널 하단에 표시되는 대시보드 위젯 */}
          <div className="md:hidden pt-4 pb-6 w-full">
            {dashboardWidgets}
          </div>
      ...
      <div className="order-1 md:order-2 flex-1 relative bg-slate-200 overflow-hidden flex flex-col h-[45vh] md:h-full z-10">
        ...
        {/* 💡 지도 렌더링 영역: 데이터가 없으면 로딩 뷰, 있으면 지도 표시! */}
        <div className="flex-1 w-full relative z-10">
          {!isDataReady ? (
            <div className={`w-full h-full flex flex-col items-center justify-center gap-3 ${isDarkMode ? 'bg-[#0F172A] text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
              <div className="w-8 h-8 rounded-full border-4 border-emerald-500/20 border-t-emerald-500 animate-spin" />
              <span className="text-[11px] font-bold animate-pulse">경로 분석 및 지도 세팅 중...</span>
            </div>
          ) : (
            <Maps 
        ...
        {/* 데스크탑에서만 우하단에 플로팅되는 대시보드 위젯 */}
        <div className="hidden md:flex absolute bottom-5 right-5 z-30 flex-col gap-3.5 max-w-[340px] w-full animate-in slide-in-from-bottom-5 duration-500">
          {dashboardWidgets}
        </div>
      </div>
          ...
          <div className={`w-full max-w-[420px] rounded-[2rem] md:rounded-3xl p-5 md:p-6 shadow-2xl border flex flex-col gap-3.5 md:gap-4 text-left animate-in zoom-in-95 duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-100 text-slate-800'}`}>
            <div className="flex items-center justify-between border-b pb-3 border-slate-500/10">
              <div className="flex items-center gap-2 min-w-0 pr-2">
                <div className="w-6 h-6 md:w-7 md:h-7 rounded-md md:rounded-lg bg-emerald-500 text-white flex items-center justify-center shrink-0"><Info size={14} className="md:w-4 md:h-4" /></div>
                <h3 className="font-black text-sm md:text-base tracking-tight truncate">{selectedPoi.name}</h3>
              </div>
              <button onClick={() => setSelectedPoi(null)} className={`p-1.5 rounded-xl transition-all hover:scale-95 shrink-0 ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700 text-slate-400' : 'bg-slate-50 hover:bg-slate-100 text-slate-500'}`}>
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-3 md:space-y-3.5 text-[11px] md:text-xs font-semibold text-slate-400">
              <div className="flex items-center gap-2 md:gap-3">
                <span className="w-20 md:w-24 shrink-0 font-black tracking-wider uppercase text-[9px] md:text-[10px]">예측 혼잡도</span>
              ...
              <div className="flex items-center gap-2 md:gap-3">
                <span className="w-20 md:w-24 shrink-0 font-black tracking-wider uppercase text-[9px] md:text-[10px]">예상 대기</span>
                <div className={`flex items-center gap-1.5 font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}><Clock size={12} className="opacity-60 md:w-3.5 md:h-3.5" /> {Math.round(selectedPoi.t_wait)}분</div>
              </div>
              <div className="flex items-center gap-2 md:gap-3">
                <span className="w-20 md:w-24 shrink-0 font-black tracking-wider uppercase text-[9px] md:text-[10px]">출발지 이동</span>
                <div className={`flex items-center gap-1.5 font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}><Navigation size={12} className="opacity-60 md:w-3.5 md:h-3.5" /> {Math.round(selectedPoi.t_travel)}분</div>
              </div>
              <div className="flex items-center gap-2 md:gap-3">
                <span className="w-20 md:w-24 shrink-0 font-black tracking-wider uppercase text-[9px] md:text-[10px]">코스 완주율</span>
            ...
            <button 
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-black py-3 md:py-3.5 rounded-xl md:rounded-2xl text-[11px] md:text-xs tracking-wider uppercase shadow-md transition-all flex items-center justify-center gap-1.5 mt-1"
            >
              <RefreshCw size={12} className="md:w-[13px] md:h-[13px]" /> 이 장소로 목적지 변경
            </button>
          </div>
        </div>
      )}
